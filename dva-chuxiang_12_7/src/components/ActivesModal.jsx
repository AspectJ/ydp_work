import React from 'react'
import { Modal, Form, Input, Radio, Upload, Icon, Button, message, DatePicker } from 'antd'
import moment from 'moment';
import { baseUrl } from '../services/requests'

const FormItem = Form.Item;
const RadioButton = Radio.Button;
const RadioGroup = Radio.Group;
const { MonthPicker, RangePicker } = DatePicker;
const dateFormat = 'YYYY/MM/DD';

// umeditor
import Editor from 'react-umeditor'
const icons = [
  "bold italic underline strikethrough fontborder emphasis | ",
  "paragraph fontfamily fontsize | superscript subscript | ",
  "forecolor backcolor |",
  "cleardoc  | indent outdent | justifyleft justifycenter justifyright | touppercase tolowercase | ",
  "horizontal date time  | image spechars | inserttable"
];
const plugins = {
  image: {
  	uploader: {
			url: baseUrl + '/rest/image/uploadContentImage',
			name: "file",
			request: "relativePath"
    }
  }
};


// 图片上传后响应的路径
let responseThemeUrl = '',
    uploadThemeUrl = baseUrl + '/rest/image/newsThemeImgUpload';

let ActivesModal = React.createClass({
  getInitialState(){
    return {
      html: this.props.item.acti_content || '请输入...',
      start_time: this.props.item.start_time,
      end_time:	this.props.item.end_time
    };
  },
  onDateChange(dates, dateString) {
    this.setState({
      html: this.refs.editor.getContent(),
    	start_time: dateString[0] + " 00:00:00",
    	end_time:	dateString[1] + " 00:00:00"
    });
  },
  handleFormChange(){
    this.setState({
			html: this.refs.editor.getContent()
		});
	},
  handleUpload(info){
    this.setState({
			html: this.refs.editor.getContent()
		});

    if (info.file.status !== 'uploading') {
      console.log(info.file, info.fileList);
    }
    if (info.file.status === 'done') {
      message.success(`${info.file.name} 上传成功`);
      responseThemeUrl = (info.file.response.data.picUrl).replace(/\\/g, "/");
    } else if (info.file.status === 'error') {
      message.error(`${info.file.name} 上传失败`);
    }
  },
  render(){
    const that = this,
          ModalOpts = {
      title: this.props.modalType === 'create' ? '新建活动' : this.props.modalType === 'update' ? '编辑活动' : '查看活动',
      visible: this.props.modalVisible,
      maskClosable: false,
      width: '800px',
      wrapClassName: "rightModal",
      onOk(){
        if( that.props.modalType === 'visit' ){
          return that.props.onClose();
        } else {
          // 富文本BUG 需要先保存至state中
          that.setState({
      			html: that.refs.editor.getContent()
      		});

          that.props.form.validateFields((err, values) => {
            if(err) return;

            let resultUrl = '',
                fileList = that.props.form.getFieldValue('org_path');

            if(typeof(fileList) === 'object' ){
              if( fileList.fileList.length > 1 ){
                return message.error('只能上传一张图片', 3);
              } else if( fileList.fileList.length === 0 ) {
                resultUrl   = '';
                responseThemeUrl = '';
              } else {
                resultUrl = responseThemeUrl ? ('/cx' + responseThemeUrl) : '';
                responseThemeUrl = '';
              }
            } else {
              resultUrl = !!fileList ? fileList : '';
            }

            if( !!!that.state.html || that.refs.editor.getContent() === '请输入...' ){
              return message.warning("活动内容不能为空", 3);
            }
            if( !!!that.state.start_time && !!!that.state.start_time){
              return message.warning("时间不能为空", 3);
            }

            if( that.props.modalType === 'create' ){
              that.props.onSubmit({
              	cinemaid: sessionStorage.getItem('cinemaid'),
          			acti_title: values.acti_title,
          			acti_status: values.acti_status,
          			acti_img: resultUrl,
          			acti_content: encodeURIComponent(that.refs.editor.getContent()),
          			author: sessionStorage.getItem("adminname"),
          			start_time:that.state.start_time,
      					end_time:that.state.end_time,
      					adminname: sessionStorage.getItem('adminname'),
              });
            } else {
              that.props.onSubmit({
              	acti_id: that.props.item.acti_id,
          			acti_title: values.acti_title,
          			acti_status: values.acti_status,
          			org_path: resultUrl,
          			acti_content: encodeURIComponent(that.refs.editor.getContent()),
          			image_change: 0,
          			imageid: that.props.item.acti_img,
          			adminname: sessionStorage.getItem('adminname'),
          			start_time:that.state.start_time,
  	            end_time:that.state.end_time
              });
            }

          });
        }
      },
      onCancel(){
        that.props.onClose();
      }
    };

    const { getFieldDecorator, validateFields } = this.props.form;

    const formItemLayout = {
      labelCol: { span: 3 },
      wrapperCol: { span: 20 }
    };

    const uploadProps = {
      action: uploadThemeUrl,
      data: { oldImg: "activity" },
      accept: ".png,.gif,.jpeg,.jpg,.bmp",
      listType: "picture",
      onChange: this.handleUpload,
      defaultFileList: this.props.item.org_path ? [{
        uid: -1,
        name: '当前主题图',
        status: 'done',
        url: this.props.item.org_path,
        thumbUrl: this.props.item.org_path
      }] : []
    }

    return (
      <Modal {...ModalOpts}>
        <Form horizontal>
          <FormItem
            {...formItemLayout}
            label="活动标题"
          >
            {
              this.props.modalType === 'visit' ?
                <p className="ant-form-text">{this.props.item.acti_title}</p> :
                getFieldDecorator('acti_title', {
                  initialValue: this.props.item.acti_title,
                  rules: [
                    { required: true, message: '活动标题不能为空' },
                    { max: 40, message: '活动标题不能超过40个字' }
                  ]
                })(
                  <Input type="text" size="large" style={{ width: 450 }} placeholder="请输入活动标题" onChange={this.handleFormChange} />
                )
            }
          </FormItem>
          <FormItem
            {...formItemLayout}
            label="活动状态"
          >
            {
              this.props.modalType === 'visit' ?
                <p className="ant-form-text">
                  {
                    this.props.item.acti_status === 0 ? '正式发布' :
                    this.props.item.acti_status === 1 ? '不可查阅' :
                    this.props.item.acti_status === 2 ? '草稿' : ''
                  }
                </p> :
                getFieldDecorator('acti_status', {
                  initialValue: !!this.props.item.acti_status ? this.props.item.acti_status : 0
                })(
                  <RadioGroup size="large" onChange={this.handleFormChange}>
                    <RadioButton value={0}>正式发布</RadioButton>
                    <RadioButton value={1}>不可查阅</RadioButton>
                    <RadioButton value={2}>草稿</RadioButton>
                  </RadioGroup>
                )
            }
          </FormItem>
		  <FormItem
	          label="活动时间:"
	          {...formItemLayout}
	      >
		  {
		  	this.props.modalType === 'create' ?
		  		<RangePicker
	        	format={dateFormat}
	        	onChange={this.onDateChange}
	       	/> :
	       	<RangePicker
	       		defaultValue={[moment(this.props.item.start_time, dateFormat), moment(this.props.item.end_time, dateFormat)]}
	        	format={dateFormat}
	        	onChange={this.onDateChange}
	        	disabled={this.props.modalType === 'visit' ? true : false}
	       	/>
		  }
	      </FormItem>
          <FormItem
            {...formItemLayout}
            label="主题照片"
            extra="只能上传一张图片，建议图片尺寸：1190*90(像素)"
          >
            {
              this.props.modalType === 'visit' ?
                this.props.item.org_path ? <p><img className="ThemePic" src={this.props.item.org_path} alt="主题照片"/></p> : <p style={{color: '#999'}}>该新闻暂无主题照片</p> :
                getFieldDecorator('org_path', {
                  initialValue: this.props.item.org_path
                })(
                  <Upload {...uploadProps} className="upload-list-inline">
                    <Button type="ghost">
                      <Icon type="upload" /> 点击上传
                    </Button>
                  </Upload>
                )
            }
          </FormItem>
          <FormItem
            {...formItemLayout}
            label="活动内容"
          >
            {
              this.props.modalType === 'visit' ?
                <div dangerouslySetInnerHTML={{__html: this.props.item.acti_content}}></div> :
                <Editor ref="editor" icons={icons} plugins={plugins} defaultValue={this.state.html} />
            }
          </FormItem>
        </Form>
      </Modal>
    );
  }
});

 export default Form.create()(ActivesModal)
