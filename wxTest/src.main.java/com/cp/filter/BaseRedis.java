...
public class BaseRedis
{
	...
	public String queryMapField(int database, String key, String field)
	{
		...
		{
			MyMongo.mErrorLog("查询Map的字段信息", e);
			return null;
		}
	}
	...
	protected void updateMapField(int database, String key, String field, String value)
	{
		...
		{
			MyMongo.mErrorLog("Redis-修改map字段", e);
		}
	}
	...
	public void removeKey(int database, String key)
	{
		...
		{
			MyMongo.mErrorLog("Redis-移除key", e);
		}
	}
	 ...
	 * @return
	 */
	public List<String> queryList(int database, String key)
	{
		try
		{
			RedisConnection conn = jedisConnectionFactory.getConnection();
			...
			List<String> fieldValuelist = toStringList(conn.lRange(keyByte, 0, -1));
			conn.close();
			return fieldValuelist;
		} catch (Exception e)
		{
			MyMongo.mErrorLog("查询List的数据信息", e);
		}
		return null;
	}
}
