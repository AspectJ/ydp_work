...
import com.service.api.entity.ApiTicketDiv;
 ...
 */
@Component
public class ApiOtherService extends BaseService implements IService {
	...
	public void service(Param pm) throws Exception {
		...
		if (SystemConstant.jsonType == dataPackType) {
			...
			if ("select".equals(operType)) {// 加载票版
				select(pm, obj);
			}else {
	...
	//加载票版
	private void select(Param pm,JSONObject obj) throws Exception{
		Map<String, Object> retMap = new HashMap<String, Object>();
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			String sessionsid=obj.getString("sessionsid");
			map.put("sessionsid", sessionsid);
			ApiTicketDiv divObj=(ApiTicketDiv)getDao().queryForObject("apiother.queryApiOther", map, ApiTicketDiv.class);
			if (null != divObj) {
				retMap.put("data", divObj.getContentStr());
			} else {
				retMap.put("data", "");
			}
			retMap.put(TypeConstant.SF, TypeConstant.SUCCESS);
			retMap.put(TypeConstant.SHOW_MSG, "加载成功！");
			pm.setData(retMap);
		} catch (Exception e) {
			retMap.put(TypeConstant.SF, TypeConstant.FAIL);
			retMap.put(TypeConstant.SHOW_MSG, "加载失败！");
			pm.setData(retMap);
