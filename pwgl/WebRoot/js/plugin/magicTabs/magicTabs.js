...
mac.tabs = function(self, cfg) {
	...
	self.adjust = function() {
		...
		if (hd.width()) {
			...
			//alert(hd.width()+"==="+ht.width());
			var b = hd.width()-80<=ht.width();
			//=======任托宇修改记录：2012-12-04====================================================//
			hl.toggle(b);
			hr.toggle(b);
...
//		self.adjust();
	...
	self.addTab = function(p, n, cb) {
		...
		if (p.update && a.length > 0) {
			...
			b = $('<div onscroll="currentTabScroll()" class="main hidden" id="' + k + '" name="' + k + '"></div>');
			if (p.bodyCls)
			...
			self.adjust();
			var m = $('<div id="'+k+'_TabMain" class="main"></div>');
			m.append(p.title);
			var bw = 0;// xbtn width
			a.append('<div id="'+k+'_TabLeft" class="left"></div>').append(m);
			if (p.closeable) {
				var x = $('<span class="icon icon-close"></span>');
				var w = $('<div id="'+k+'_closeBtn" class="xbtn"></div>').append(x);
				x.click(function() {
					closeTab(a, b, k);
					return false;
				});
			...
			a.append('<div id="'+k+'_TabRight" class="right"></div>');
			bd.append(b);
		}
		a.unbind('click');
		a.click(function() {
