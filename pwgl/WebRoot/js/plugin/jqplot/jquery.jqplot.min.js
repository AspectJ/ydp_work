...
(function(H) {
	...
	H.jqplot = function(ah, ae, ac) {
		...
		if (H.jqplot.config.catchErrors) {
			try {
				...
				H("#" + ah).append('<div class="jqplot-error-message">' + ai
						+ "</div>");
	...
	H.jqplot.config = {
		...
		defaultColors : ["#4bb2c5", "#EAA228", "#c5b47f", "#579575", "#839557",
	...
	H.jqplot.use_excanvas = (H.browser.msie && !H.jqplot.support_canvas())
			? true
			: false;
	...
	H.jqplot.ElemContainer.prototype.createElement = function(ae, ag, ac, ad,
			ah) {
		this._offsets = ag;
		var ab = ac || "jqplot";
	...
	s.prototype.resetDataBounds = function() {
		...
		for (var af = 0; af < this._series.length; af++) {
			...
			if (aj.show || this.scaleToHiddenSeries) {
				ag = aj._plotData;
				if (aj._type === "line" && aj.renderer.bands.show
						&& this.name.charAt(0) !== "x") {
					ag = [[0, aj.renderer.bands._min],
				...
				for (var ae = 0, ac = ag.length; ae < ac; ae++) {
					if (this.name == "xaxis" || this.name == "x2axis") {
						if ((ag[ae][0] != null && ag[ae][0] < ai.min)
								|| ai.min == null) {
							ai.min = ag[ae][0]
						}
						if ((ag[ae][0] != null && ag[ae][0] > ai.max)
								|| ai.max == null) {
							ai.max = ag[ae][0]
						}
					} else {
						if ((ag[ae][ab] != null && ag[ae][ab] < ai.min)
								|| ai.min == null) {
							ai.min = ag[ae][ab]
						}
						if ((ag[ae][ah] != null && ag[ae][ah] > ai.max)
								|| ai.max == null) {
							ai.max = ag[ae][ah]
						}
					}
				...
				if (ad && aj.renderer.constructor !== H.jqplot.BarRenderer) {
					...
					if (ad && this._options.hasOwnProperty("forceTickAt0")
							&& this._options.forceTickAt0 == false) {
						ad = false
					} else {
						if (ad
								&& aj.renderer.constructor === H.jqplot.BarRenderer) {
							if (aj.barDirection == "vertical"
									&& this.name != "xaxis"
									&& this.name != "x2axis") {
		...
		if (ad && this.renderer.constructor === H.jqplot.LinearAxisRenderer
				&& ai.min >= 0) {
			this.padMin = 1;
	...
	O.prototype.init = function(ae, ai, ag) {
		...
		if (this.fillAlpha) {
			...
			this.fillColor = "rgba(" + ac[0] + "," + ac[1] + "," + ac[2] + ","
					+ this.fillAlpha + ")"
		}
		if (H.isFunction(this.renderer)) {
			this.renderer = new this.renderer()
	...
	O.prototype.draw = function(ah, ae, ag) {
		...
		if (this.show) {
			...
			if (!ac.preventJqPlotSeriesDrawTrigger) {
				H(ah.canvas).trigger("jqplotSeriesDraw",
	...
	O.prototype.toggleDisplay = function(ac, ae) {
		...
		if (ad) {
			if (ab.canvas._elem.is(":hidden") || !ab.show) {
				...
				ab.canvas._elem.nextAll(".jqplot-point-label.jqplot-series-"
						+ ab.index).fadeIn(ad)
			} else {
				ab.show = false;
	...
	H.jqplot.GenericCanvas.prototype.createElement = function(af, ad, ac, ag) {
		...
		ae.width = this._plotDimensions.width - this._offsets.left
				- this._offsets.right;
		ae.height = this._plotDimensions.height - this._offsets.top
	...
	var Q = ["yMidAxis", "xaxis", "yaxis", "x2axis", "y2axis", "y3axis",
	...
	function N() {
		...
		this.init = function(ao, al, aq) {
			...
			if (al == null || H.isArray(al) == false || al.length == 0
					|| H.isArray(al[0]) == false || al[0].length == 0) {
			...
			for (var am = 0; am < this.series.length; am++) {
				...
				for (var ak = 0; ak < H.jqplot.preSeriesInitHooks.length; ak++) {
					H.jqplot.preSeriesInitHooks[ak].call(this.series[am], ao,
							this.data, this.options.seriesDefaults,
							this.options.series[am], this)
				}
				for (var ak = 0; ak < this.preSeriesInitHooks.hooks.length; ak++) {
					this.preSeriesInitHooks.hooks[ak].call(this.series[am], ao,
							this.data, this.options.seriesDefaults,
							this.options.series[am], this)
				}
				...
				for (var ak = 0; ak < H.jqplot.postSeriesInitHooks.length; ak++) {
					H.jqplot.postSeriesInitHooks[ak].call(this.series[am], ao,
							this.data, this.options.seriesDefaults,
							this.options.series[am], this)
				}
				for (var ak = 0; ak < this.postSeriesInitHooks.hooks.length; ak++) {
					this.postSeriesInitHooks.hooks[ak].call(this.series[am],
							ao, this.data, this.options.seriesDefaults,
							this.options.series[am], this)
				}
			...
			for (var am = 0, aj = Q.length; am < aj; am++) {
				...
				if (this.axes[ag].borderColor == null) {
					if (ag.charAt(0) !== "x" && ai.useSeriesColor === true
							&& ai.show) {
		...
		this.reInitialize = function(an, af) {
			...
			for (var ao = 0, ak = this.series.length; ao < ak; ao++) {
				...
				for (var am = 0; am < H.jqplot.preSeriesInitHooks.length; am++) {
					H.jqplot.preSeriesInitHooks[am].call(this.series[ao], ap,
							this.data, this.options.seriesDefaults,
							this.options.series[ao], this)
				}
				for (var am = 0; am < this.preSeriesInitHooks.hooks.length; am++) {
					this.preSeriesInitHooks.hooks[am].call(this.series[ao], ap,
							this.data, this.options.seriesDefaults,
							this.options.series[ao], this)
				}
				...
				for (var am = 0; am < H.jqplot.postSeriesInitHooks.length; am++) {
					H.jqplot.postSeriesInitHooks[am].call(this.series[ao], ap,
							this.data, this.options.seriesDefaults,
							this.options.series[ao], this)
				}
				for (var am = 0; am < this.postSeriesInitHooks.hooks.length; am++) {
					this.postSeriesInitHooks.hooks[am].call(this.series[ao],
							ap, this.data, this.options.seriesDefaults,
							this.options.series[ao], this)
				}
		...
		this.quickInit = function() {
			...
			for (var ai = 0; ai < this.series.length; ai++) {
				if (this.series[ai]._type === "line"
						&& this.series[ai].renderer.bands.show) {
					this.series[ai].renderer.initBands.call(this.series[ai],
							this.series[ai].renderer.options, this)
				}
		...
		function ab(aj) {
			...
			for (var ak = 0; ak < aj.length; ak++) {
				var ag;
				var al = [aj[ak].data, aj[ak]._stackData, aj[ak]._plotData,
						aj[ak]._prevPlotData];
		...
		this.parseOptions = function(ar) {
			...
			this.defaultAxisStart = (ar && ar.defaultAxisStart != null)
					? ar.defaultAxisStart
			...
			for (var am = 0; am < this.data.length; am++) {
				...
				for (var al = 0; al < H.jqplot.preParseSeriesOptionsHooks.length; al++) {
					H.jqplot.preParseSeriesOptionsHooks[al].call(aq,
							this.options.seriesDefaults,
							this.options.series[am])
				}
				for (var al = 0; al < this.preParseSeriesOptionsHooks.hooks.length; al++) {
					this.preParseSeriesOptionsHooks.hooks[al].call(aq,
							this.options.seriesDefaults,
							this.options.series[am])
				}
				H.extend(true, aq, af);
				var ai = "vertical";
				if (aq.renderer === H.jqplot.BarRenderer && aq.rendererOptions
						&& aq.rendererOptions.barDirection == "horizontal") {
					ai = "horizontal";
					aq._stackAxis = "x";
				...
				for (var al = 0; al < H.jqplot.postParseSeriesOptionsHooks.length; al++) {
					H.jqplot.postParseSeriesOptionsHooks[al].call(
							this.series[am], this.options.seriesDefaults,
							this.options.series[am])
				}
				for (var al = 0; al < this.postParseSeriesOptionsHooks.hooks.length; al++) {
					this.postParseSeriesOptionsHooks.hooks[al].call(
							this.series[am], this.options.seriesDefaults,
							this.options.series[am])
				}
			}
		...
		this.draw = function() {
			if (this.drawIfHidden || this.target.is(":visible")) {
				...
				for (aB = 0, ay = this.preDrawHooks.length; aB < ay; aB++) {
					this.preDrawHooks.hooks[aB].apply(this,
							this.preDrawSeriesHooks.args[aB])
				}
				...
				var aA = ["y2axis", "y3axis", "y4axis", "y5axis", "y6axis",
				...
				if (this.options.gridDimensions
						&& H.isPlainObject(this.options.gridDimensions)) {
					var am = parseInt(this.options.gridDimensions.width, 10)
							|| 0;
					var aC = parseInt(this.options.gridDimensions.height, 10)
							|| 0;
				...
				al.xaxis.pack({
							position : "absolute",
							bottom : this._gridPadding.bottom
									- al.xaxis.getHeight(),
							left : 0,
							width : this._width
						}, {
				...
				var an = (this._width - this._gridPadding.left - this._gridPadding.right)
						/ 2
						+ this._gridPadding.left
						- al.yMidAxis.getWidth()
						/ 2;
				al.yMidAxis.pack({
							position : "absolute",
							top : 0,
				...
				for (aB = 0, ay = aD; aB < ay; aB++) {
					...
					this.target.append(ak[az].shadowCanvas.createElement(
				...
				for (aB = 0, ay = aD; aB < ay; aB++) {
					az = this.seriesStack[aB];
					this.target.append(ak[az].canvas.createElement(
				...
				for (var aB = 0, ay = H.jqplot.eventListenerHooks.length; aB < ay; aB++) {
					this.eventCanvas._elem.bind(
							H.jqplot.eventListenerHooks[aB][0], {
								plot : this
							}, H.jqplot.eventListenerHooks[aB][1])
				}
				for (var aB = 0, ay = this.eventListenerHooks.hooks.length; aB < ay; aB++) {
					this.eventCanvas._elem.bind(
							this.eventListenerHooks.hooks[aB][0], {
								plot : this
							}, this.eventListenerHooks.hooks[aB][1])
				}
				var aq = this.fillBetween;
				if (aq.fill && aq.series1 !== aq.series2 && aq.series1 < aD
						&& aq.series2 < aD && ak[aq.series1]._type === "line"
						&& ak[aq.series2]._type === "line") {
				...
				for (var aB = 0, ay = this.postDrawHooks.hooks.length; aB < ay; aB++) {
					this.postDrawHooks.hooks[aB].apply(this,
							this.postDrawHooks.args[aB])
				}
				...
				for (aB = 0, ay = aD; aB < ay; aB++) {
					...
					if (ap.animation && ap.animation._supported
							&& ap.animation.show
		...
		function ac(ao) {
			...
			var ak = ["xaxis", "yaxis", "x2axis", "y2axis", "y3axis", "y4axis",
					"y5axis", "y6axis", "y7axis", "y8axis", "y9axis",
					"yMidAxis"];
			var af = am.axes;
			var ag, ah;
			for (ag = 11; ag > 0; ag--) {
				...
				switch (aI.renderer.constructor) {
					case H.jqplot.BarRenderer :
						...
						for (aO = 0; aO < aI._barPoints.length; aO++) {
							aB = aI._barPoints[aO];
							aK = aI.gridData[aO];
							if (aD > aB[0][0] && aD < aB[2][0] && aC > aB[2][1]
									&& aC < aB[0][1]) {
								return {
					...
					case H.jqplot.FunnelRenderer :
						aD = af.x;
						aC = af.y;
						var aF = aI._vertices, aj = aF[0], ai = aF[aF.length
								- 1], am, ay, ar;
						...
						for (aO = 0; aO < aF.length; aO++) {
							ar = aF[aO];
							if (aC >= ar[0][1] && aC <= ar[3][1] && aD >= am[0]
									&& aD <= ay[0]) {
								return {
									seriesIndex : aI.index,
									pointIndex : aO,
					...
					case H.jqplot.LineRenderer :
						...
						aJ = aI.renderer;
						if (aI.show) {
							if ((aI.fill || (aI.renderer.bands.show && aI.renderer.bands.fill))
									&& (!ag.plugins.highlighter || !ag.plugins.highlighter.show)) {
								var aq = false;
								if (aD > aI._boundingBox[0][0]
										&& aD < aI._boundingBox[1][0]
										&& aC > aI._boundingBox[1][1]
										&& aC < aI._boundingBox[0][1]) {
								...
								aH = aI.markerRenderer.size / 2
										+ aI.neighborThreshold;
								ah = (aH > 0) ? aH : 0;
								for (var aO = 0; aO < aI.gridData.length; aO++) {
									aK = aI.gridData[aO];
	...
	H.jqplot.hex2rgb = function(ad, ab) {
		ad = ad.replace("#", "");
		if (ad.length == 3) {
			ad = ad.charAt(0) + ad.charAt(0) + ad.charAt(1) + ad.charAt(1)
					+ ad.charAt(2) + ad.charAt(2)
		}
		var ac;
		ac = "rgba(" + parseInt(ad.slice(0, 2), 16) + ", "
				+ parseInt(ad.slice(2, 4), 16) + ", "
				+ parseInt(ad.slice(4, 6), 16);
		if (ab) {
			ac += ", " + ab
		}
		ac += ")";
	...
	H.jqplot.AxisLabelRenderer.prototype.draw = function(ab, ac) {
		...
		this._elem = H('<div style="position:absolute;" class="jqplot-'
				+ this.axis + '-label"></div>');
	...
	H.jqplot.CanvasGridRenderer.prototype.draw = function() {
		...
		var ap = this._axes;
		am.save();
		am.clearRect(0, 0, this._plotDimensions.width,
		...
		for (var aq = 4; aq > 0; aq--) {
			...
			if (ab.show) {
				if (ab.drawBaseline) {
					...
					switch (aw) {
						case "xaxis" :
							ai(this._left, this._bottom, this._right,
									this._bottom, av);
							break;
						case "yaxis" :
						...
						case "x2axis" :
							...
							break;
						case "y2axis" :
							ai(this._right, this._bottom, this._right,
									this._top, av);
							break
					}
				}
					...
					if (ah.show) {
						...
						switch (aw) {
							case "xaxis" :
								if (ah.showGridline
										&& this.drawGridlines
										&& ((!ah.isMinorTick && ab.drawMajorGridlines) || (ah.isMinorTick && ab.drawMinorGridlines))) {
									ai(ae, this._top, ae, this._bottom)
								}
								if (ah.showMark
										&& ah.mark
										&& ((!ah.isMinorTick && ab.drawMajorTickMarks) || (ah.isMinorTick && ab.drawMinorTickMarks))) {
									aj = ah.markSize;
									ak = ah.mark;
									var ae = Math.round(ab.u2p(ah.value)) + 0.5;
									switch (ak) {
		...
		ag = ["y3axis", "y4axis", "y5axis", "y6axis", "y7axis", "y8axis",
				"y9axis", "yMidAxis"];
		for (var aq = 7; aq > 0; aq--) {
			...
			if (ab.show) {
				...
				var af = au[0];
				var ad = ab.getLeft();
				var ao = [[ad, ac.getTop() + ac.getHeight() / 2],
		...
		if (this.shadow) {
			var ao = [[this._left, this._bottom], [this._right, this._bottom],
	...
	H.jqplot.LineRenderer.prototype.init = function(ac, ah) {
		...
		if (ad == null) {
			if (this.lineWidth > 2.5) {
				ad = 1.25
						* (1 + (Math.atan((this.lineWidth / 2.5)) / 0.785398163 - 1)
								* 0.6)
			} else {
		...
		if (!this.isTrendline && this.fill || this.renderer.bands.show) {
			...
			if (!this.highlightColor) {
				var ae = (this.renderer.bands.show)
	...
	H.jqplot.LineRenderer.prototype.initBands = function(ae, ao) {
		...
		if (ah.fillColor === null) {
			...
			ar[3] = ar[3] * 0.5;
			ah.fillColor = "rgba(" + ar[0] + ", " + ar[1] + ", " + ar[2] + ", "
					+ ar[3] + ")"
		}
	};
	function G(ac, ab) {
	...
	function k(ad, ac) {
		var ab = Math.sqrt(Math.pow((ac[0] - ad[0]), 2)
				+ Math.pow((ac[1] - ad[1]), 2));
	...
	function F(aD) {
		...
		for (var af = 1, av = aD.length; af < av; af++) {
			...
			for (var aw = 0; aw < 2; aw++) {
				...
				if (ay == 0 || ay == ab) {
					...
					if (ap[ay + 1] - ap[ay] == 0 || ap[ay] - ap[ay - 1] == 0) {
						...
						if (((ae[ay + 1] - ae[ay]) / (ap[ay + 1] - ap[ay]) + (ae[ay] - ae[ay
								- 1])
								/ (ap[ay] - ap[ay - 1])) == 0) {
							ac[aw] = 0
						} else {
			...
			if (af == 1) {
				...
				if (af == ab) {
					ac[1] = 3 / 2 * (ap[ab] - ap[ab - 1])
							/ ao(ae[ab], ae[ab - 1]) - ac[0] / 2
				}
			}
			an[0] = -2 * (ac[1] + 2 * ac[0]) / ao(ae[af], ae[af - 1]) + 6
					* (ap[af] - ap[af - 1])
					/ Math.pow(ao(ae[af], ae[af - 1]), 2);
			an[1] = 2 * (2 * ac[1] + ac[0]) / ao(ae[af], ae[af - 1]) - 6
					* (ap[af] - ap[af - 1])
					/ Math.pow(ao(ae[af], ae[af - 1]), 2);
			aj = 1 / 6 * (an[1] - an[0]) / ao(ae[af], ae[af - 1]);
			ak = 1 / 2 * (ae[af] * an[0] - ae[af - 1] * an[1])
					/ ao(ae[af], ae[af - 1]);
			al = (ap[af] - ap[af - 1] - ak
					* (Math.pow(ae[af], 2) - Math.pow(ae[af - 1], 2)) - aj
					* (Math.pow(ae[af], 3) - Math.pow(ae[af - 1], 3)))
					/ ao(ae[af], ae[af - 1]);
			aq = ap[af - 1] - al * ae[af - 1] - ak * Math.pow(ae[af - 1], 2)
					- aj * Math.pow(ae[af - 1], 3);
			...
			for (var aw = 0, au = az; aw < au; aw++) {
				...
				ai = ae[af - 1] + aw * aC;
				aB.push(ai);
				aB.push(aq + al * ai + ak * Math.pow(ai, 2) + aj
						* Math.pow(ai, 3));
				ad.push(aB);
	...
	H.jqplot.LineRenderer.prototype.setGridData = function(aj) {
		...
		for (var ah = 0, ad = ag.length; ah < ad; ah++) {
			if (ag[ah][0] != null && ag[ah][1] != null) {
				this.gridData.push([af.call(this._xaxis, ag[ah][0]),
						ab.call(this._yaxis, ag[ah][1])])
			} else {
				if (ag[ah][0] == null) {
					ac = true;
			...
			if (ak[ah] != null && ak[ah][0] != null && ak[ah][1] != null) {
				this._prevGridData.push([af.call(this._xaxis, ak[ah][0]),
						ab.call(this._yaxis, ak[ah][1])])
			} else {
				if (ak[ah] != null && ak[ah][0] == null) {
					...
					if (ak[ah] != null && ak[ah][0] != null
						...
						this._prevGridData.push([
								af.call(this._xaxis, ak[ah][0]), null])
					}
				}
			}
		}
		...
		if (this._type === "line" && ae.show) {
			for (var ah = 0, ad = ae.hiData.length; ah < ad; ah++) {
				this.renderer._hiBandGridData.push([
						af.call(this._xaxis, ae.hiData[ah][0]),
						ab.call(this._yaxis, ae.hiData[ah][1])])
			}
			for (var ah = 0, ad = ae.lowData.length; ah < ad; ah++) {
				this.renderer._lowBandGridData.push([
						af.call(this._xaxis, ae.lowData[ah][0]),
						ab.call(this._yaxis, ae.lowData[ah][1])])
			}
		}
		if (this._type === "line" && this.renderer.smooth
	...
	H.jqplot.LineRenderer.prototype.makeGridData = function(ai, ak) {
		...
		for (var ah = 0; ah < ai.length; ah++) {
			if (ai[ah][0] != null && ai[ah][1] != null) {
				al.push([ag.call(this._xaxis, ai[ah][0]),
						ab.call(this._yaxis, ai[ah][1])])
			} else {
				if (ai[ah][0] == null) {
					ac = true;
		...
		if (this._type === "line" && af.show) {
			for (var ah = 0, ae = af.hiData.length; ah < ae; ah++) {
				this.renderer._hiBandGridData.push([
						ag.call(this._xaxis, af.hiData[ah][0]),
						ab.call(this._yaxis, af.hiData[ah][1])])
			}
			for (var ah = 0, ae = af.lowData.length; ah < ae; ah++) {
				this.renderer._lowBandGridData.push([
						ag.call(this._xaxis, af.lowData[ah][0]),
						ab.call(this._yaxis, af.lowData[ah][1])])
			}
		}
					...
					if (ab) {
						...
						if (this.markerRenderer.show) {
							...
							for (aw = 0; aw < aB.length; aw++) {
								this.markerRenderer.draw(aB[aw][0], aB[aw][1],
										aq, ak.markerOptions)
							}
						}
					}
					...
					if (this.renderer.bands.show) {
						...
						if (this.renderer.bands.fill) {
							if (this.renderer.smooth) {
								...
								ag = this.renderer._hiBandGridData
			...
			if (this.markerRenderer.show && !au) {
				...
				for (aw = 0; aw < aC.length; aw++) {
					if (aC[aw][0] != null && aC[aw][1] != null) {
						this.markerRenderer.draw(aC[aw][0], aC[aw][1], aq,
								ak.markerOptions)
					}
				}
			}
	...
	function g(af, ae, ai, ah, ag) {
		if (ah) {
			...
			if (ag.series[ad[0]].highlightMouseOver
					&& !(ad[0] == ag.plugins.lineRenderer.highlightedSeriesIndex)) {
	...
	function d(ae, ad, ah, ag, af) {
		if (ag) {
			...
			if (af.series[ac[0]].highlightMouseDown
					&& !(ac[0] == af.plugins.lineRenderer.highlightedSeriesIndex)) {
				var ab = jQuery.Event("jqplotDataHighlight");
				ab.which = ae.which;
				ab.pageX = ae.pageX;
				ab.pageY = ae.pageY;
	...
	H.jqplot.LinearAxisRenderer.prototype.draw = function(ab, ai) {
		if (this.show) {
			...
			for (var ad = 0; ad < af.length; ad++) {
				ae = af[ad];
				if (ae.show && ae.showLabel
						&& (!ae.isMinorTick || this.showMinorTicks)) {
	...
	H.jqplot.LinearAxisRenderer.prototype.set = function() {
		...
		if (this.show) {
			...
			for (var ae = 0; ae < ag.length; ae++) {
				af = ag[ae];
				if (!af._breakTick && af.show && af.showLabel
						&& (!af.isMinorTick || this.showMinorTicks)) {
					if (this.name == "xaxis" || this.name == "x2axis") {
			...
			if (this.name == "xaxis") {
				...
				if (this.name == "x2axis") {
					...
					if (this.name == "yaxis") {
						...
						if (ab
								&& this._label.constructor == H.jqplot.AxisLabelRenderer) {
	...
	H.jqplot.LinearAxisRenderer.prototype.createTicks = function(ad) {
		...
		if (aE.length) {
			...
			if (this.alignTicks) {
				if (this.name === "x2axis" && ad.axes.xaxis.show) {
					...
					if (this.name.charAt(0) === "y" && this.name !== "yaxis"
							&& this.name !== "yMidAxis" && ad.axes.yaxis.show) {
			...
			if (this.min == null || this.max == null
				...
				var aJ = H.jqplot.LinearTickGenerator(a0, aC, this._scalefact,
						aq, ay, aV);
				...
				if (a0 < ap || aC > aK) {
					...
					aJ = H.jqplot.LinearTickGenerator(ap, aK, this._scalefact,
							aq, ay, aV)
				}
				this.min = aJ[0];
				this.max = aJ[1];
				...
				if (this.autoscale && this.min == null && this.max == null) {
					...
					if (aw) {
						this.numberTicks = 2
								+ Math.ceil((ab - (this.tickSpacing - 1))
										/ this.tickSpacing);
						this.min = 0;
						aB = 0;
						af = aC / (this.numberTicks - 1);
						am = Math.pow(10, Math.abs(Math.floor(Math.log(af)
										/ Math.LN10)));
						...
						if (aH) {
							this.numberTicks = 2
									+ Math.ceil((ab - (this.tickSpacing - 1))
											/ this.tickSpacing);
							var aD = Math.ceil(Math.abs(a0) / ao
									* (this.numberTicks - 1));
							...
							if (this.numberTicks == null) {
								if (this.tickInterval) {
									this.numberTicks = 3
											+ Math.ceil(ao / this.tickInterval)
								} else {
									this.numberTicks = 2
											+ Math
													.ceil((ab - (this.tickSpacing - 1))
															/ this.tickSpacing)
								}
							}
							if (this.tickInterval == null) {
								...
								if (af < 1) {
									am = Math.pow(10, Math.abs(Math.floor(Math
													.log(af)
													/ Math.LN10)))
								} else {
									am = 1
								}
								this.tickInterval = Math.ceil(af * am
										* this.pad)
										/ am
							} else {
					...
					aM = (this.min != null) ? this.min : a0 - ao
							* (this.padMin - 1);
					ar = (this.max != null) ? this.max : aC + ao
							* (this.padMax - 1);
					ao = ar - aM;
					if (this.numberTicks == null) {
						if (this.tickInterval != null) {
							this.numberTicks = Math.ceil((ar - aM)
									/ this.tickInterval)
									+ 1
						} else {
							if (ab > 100) {
				...
				if (this.renderer.constructor == H.jqplot.LinearAxisRenderer
						&& this._autoFormatString == "") {
					ao = this.max - this.min;
					var a1 = new this.tickRenderer(this.tickOptions);
					var aF = a1.formatString
							|| H.jqplot.config.defaultTickFormatString;
						...
						if (this.tickInterval < ak) {
							if (aQ == null && a4 == null) {
								...
								if (aZ == null && aB == null) {
									...
									if (aZ == null) {
										var aR = (this._dataBounds.max - this.min)
												/ this.tickInterval;
										aR = aR.toFixed(11);
										...
										if (aB == null) {
											var aR = (this.max - this._dataBounds.min)
													/ this.tickInterval;
											aR = aR.toFixed(11);
											this.numberTicks = Math.ceil(aR)
													+ 2;
											this.min = this.max
													- this.tickInterval
													* (this.numberTicks - 1)
											...
											this.numberTicks = Math
													.ceil((aZ - aB)
															/ this.tickInterval)
													+ 1;
											this.min = Math.floor(aB
													* Math.pow(10, aX))
													/ Math.pow(10, aX);
											this.max = Math.ceil(aZ
													* Math.pow(10, aX))
													/ Math.pow(10, aX);
											this.numberTicks = Math
		...
		if (this.breakPoints) {
			...
			this.u2p = function(au) {
				...
				if (au <= this.breakPoints[0]) {
					...
					return (au - this.breakPoints[1] + this.breakPoints[0] - am)
							* ae / af + ag
				}
			};
			if (this.name.charAt(0) == "x") {
				...
				this.series_u2p = function(au) {
					...
					if (au >= this.breakPoints[1]) {
						...
						return (au + this.breakPoints[1] - this.breakPoints[0] - an)
								* ae / af
					}
				};
				this.series_p2u = function(au) {
			...
			if (this.name == "xaxis" || this.name == "x2axis") {
				for (var ao = 0; ao < ar.length; ao++) {
					...
					if (aj.show && aj.showLabel) {
						var ab;
						if (aj.constructor == H.jqplot.CanvasAxisTickRenderer
								&& aj.angle) {
							...
							switch (aj.labelPosition) {
								case "auto" :
									if (aq * aj.angle < 0) {
										ab = -aj.getWidth()
												+ aj._textRenderer.height
												* Math
														.sin(-aj._textRenderer.angle)
												/ 2
									} else {
										ab = -aj._textRenderer.height
												* Math
														.sin(aj._textRenderer.angle)
												/ 2
									}
								...
								case "end" :
									ab = -aj.getWidth()
											+ aj._textRenderer.height
											* Math.sin(-aj._textRenderer.angle)
											/ 2;
									break;
								case "start" :
									ab = -aj._textRenderer.height
	...
	H.jqplot.MarkerRenderer.prototype.drawSquare = function(ad, ac, ag, af, ai) {
		...
		var aj = this.size / 2 / ab;
		var ah = this.size / 2 * ab;
		var ae = [[ad - aj, ac - ah], [ad - aj, ac + ah], [ad + aj, ac + ah],
				[ad + aj, ac - ah]];
		if (this.shadow) {
			this.shadowRenderer.draw(ag, ae)
		}
	...
	H.jqplot.ShadowRenderer.prototype.draw = function(ao, am, aq) {
		...
		ao.strokeStyle = ab.strokeStyle || this.strokeStyle || "rgba(0,0,0,"
				+ ad + ")";
		ao.fillStyle = ab.fillStyle || this.fillStyle || "rgba(0,0,0," + ad
				+ ")";
		for (var ae = 0; ae < ah; ae++) {
			var al = H.jqplot.LinePattern(ao, ak);
			ao.translate(Math.cos(this.angle * Math.PI / 180) * af, Math
	...
	H.jqplot.TableLegendRenderer.prototype.draw = function() {
		...
		if (this.show) {
			...
			for (var ah = 0; ah < ag.length; ah++) {
				ak = ag[ah];
				if (ak._stack
						|| ak.renderer.constructor == H.jqplot.BezierCurveRenderer) {
					ai = true
				}
				if (ak.show && ak.showLabel) {
					...
					for (var ae = 0; ae < H.jqplot.addLegendRowHooks.length; ae++) {
						...
						if (aj) {
							this.renderer.addrow.call(this, aj.label, aj.color,
									ab);
							ab = true
						}
					}
		...
		/**
		 * 
		 * 以下内容是后面加的，为了满足分类信息可以横排显示的需求 by liying
		 */

		// 如果需要按横排的个数显示分类信息
		if (this._elem != undefined && this._elem != null
				&& this.showLegendByRow == true && this.legendRowSize != 1
				&& this.bindLegendType != 'pie') {

			// 如果没定义单行显示数，则默认单行显示5个
			if (this.legendRowSize == null) {
				this.legendRowSize = 5;
			}

			var modSize = this.legendRowSize * 2;
			// 先找到tr下面的所有td元素
			var tds = this._elem.find('tr').find('td');

			var count = 0;

			// 将原来对象中的table元素的内容清空
			this._elem.empty("table");

			var tr = null;

			// 创建一个div
			var content = H(document.createElement("div"));
			// 创建一个table
			var tb = H(document.createElement("table"));
			// 将table加入到div中
			tb.appendTo(content);
			...
			while (true) {
				...
				// 如果能被单行显示的个数乘以2整除，则添加tr元素
				if (count % modSize == 0) {
					tr = H(document.createElement("tr"));
					tr.appendTo(tb);
				}

				// 将遍历的当前的td元素从js对象转换成jquery对象
				var td = H(tds[count]);
				// 将td添加到tr
				td.appendTo(tr);
				count++;
			...
			// 将当前对象的html内容替换成div的html内容（替换掉了原来的table内容）
			this._elem.html(content.html());
		}
		if (this._elem != undefined && this._elem != null && !this.showLegend) {
			this._elem.html("");
		}
		return this._elem
	};
	H.jqplot.TableLegendRenderer.prototype.pack = function(ad) {
			...
			if (this.placement == "insideGrid") {
				switch (this.location) {
					case "nw" :
						...
						break;
					case "n" :
						var ac = (ad.left + (this._plotDimensions.width - ad.right))
								/ 2 - this.getWidth() / 2;
						var ab = ad.top;
						this._elem.css("left", ac);
						this._elem.css("top", ab);
					...
					case "e" :
						var ac = ad.right;
						var ab = (ad.top + (this._plotDimensions.height - ad.bottom))
								/ 2 - this.getHeight() / 2;
						this._elem.css({
									right : ac,
									top : ab
		...
		// 如果需要按横排的个数显示分类信息
		if (this.showLegendByRow == true && this.legendRowSize != 1
				&& this.bindLegendType == 'pie') {
			// 如果没定义单行显示数，则默认单行显示5个
			if (this.legendRowSize == null) {
				this.legendRowSize = 5;
		...
		if (!this.themes.hasOwnProperty(au)) {
			...
			for (var ar in ao.axes) {
				...
				if (ae.show) {
					...
					at = (ag.axesStyles.borderColor != null)
							? ag.axesStyles.borderColor
							: af.borderColor;
					if (af.borderColor != null) {
						ae.borderColor = af.borderColor;
						ab = true
					}
					at = (ag.axesStyles.borderWidth != null)
							? ag.axesStyles.borderWidth
							: af.borderWidth;
	...
	H.jqplot.extend = function() {
		...
		if (typeof ag !== "object"
				&& !toString.call(ag) === "[object Function]") {
			ag = {}
		}
		for (; ae < af; ae++) {
	...
	H.fn.jqplotGetComputedFontStyle = function() {
		var ae = window.getComputedStyle
				...
				: this[0].currentStyle;
		var ac = ae["font-style"] ? ["font-style", "font-weight", "font-size",
	...
	H.fn.jqplotToImageCanvas = function(ad) {
		...
		var af = (ad.backgroundColor == null)
				? "rgb(255,255,255)"
				: ad.backgroundColor;
		...
		var ak = ag.top;
		var an = 0, al = 0;
		var ar = ["jqplot-table-legend", "jqplot-xaxis-tick",
		...
		function ap(aA, aD, ay) {
			...
			var aE = window.getComputedStyle
					? window.getComputedStyle(aA, "")
					: aA.currentStyle;
			var aC = aD + az.left + parseInt(aE.marginLeft, 10)
					+ parseInt(aE.borderLeftWidth, 10)
					+ parseInt(aE.paddingLeft, 10);
			var aF = ay + az.top + parseInt(aE.marginTop, 10)
					+ parseInt(aE.borderTopWidth, 10)
					+ parseInt(aE.paddingTop, 10);
			var aG = ah.width;
			if ((aH == "div" || aH == "span")
					&& !H(aA).hasClass("jqplot-highlighter-tooltip")) {
				...
				if (aH === "table" && H(aA).hasClass("jqplot-table-legend")) {
					...
					ae
							.fillRect(aC, aF, H(aA).innerWidth(), H(aA)
											.innerHeight());
					if (parseInt(H(aA).css("border-top-width"), 10) > 0) {
						ae.strokeRect(aC, aF, H(aA).innerWidth(), H(aA)
										.innerHeight())
					}
					H(aA).find("div.jqplot-table-legend-swatch-outline").each(
							function() {
								var aO = H(this);
								ae.strokeStyle = aO.css("border-top-color");
								var aK = aC + aO.position().left;
								var aL = aF + aO.position().top;
								ae.strokeRect(aK, aL, aO.innerWidth(), aO
												.innerHeight());
								aK += parseInt(aO.css("padding-left"), 10);
								aL += parseInt(aO.css("padding-top"), 10);
								var aN = aO.innerHeight() - 2
										* parseInt(aO.css("padding-top"), 10);
								var aJ = aO.innerWidth() - 2
										* parseInt(aO.css("padding-left"), 10);
								var aM = aO
					...
					H(aA).find("td.jqplot-table-legend-label").each(function() {
						...
						var aK = aF + aL.position().top
								+ parseInt(aL.css("padding-top"), 10);
	...
	aa.prototype.getAbbrMonthName = function() {
		return aa.regional[this.locale]["monthNamesShort"][this.proxy
				.getMonth()]
	};
	...
	aa.prototype.getIsoWeek = function() {
		...
		if (ac == 53
				&& (new Date("" + ae.getFullYear() + "/12/31")).getDay() < 4) {
			ac = 1
		} else {
			if (ac === 0) {
	...
	aa.prototype.strftime = function(ab) {
		ab = ab || this.formatString
				|| aa.regional[this.locale]["formatString"];
	...
	var A = {
		...
		year : {
			add : function(ac, ab) {
				ac.setYear(ac.getFullYear()
						+ Math[ab > 0 ? "floor" : "ceil"](ab))
			},
			diff : function(ac, ab) {
	...
	aa.createDate = function(ad) {
		...
		ai = ai.replace(/^(3[01]|[0-2]?\d)[-\/]([a-z]{3,})[-\/](\d{4})/i,
				"$1 $2 $3");
	...
	aa.matchers = [
			...
			[/([1-9]\d{3})\s*-\s*(1[0-2]|0\d)\s*-\s*(3[01]|[0-2]\d)/,
				...
				if (ac) {
					...
					if (ac[6]) {
						ab = ac[6].toLowerCase() == "am"
								? (ab == 12 ? 0 : ab)
								: (ab == 12 ? 12 : ab + 12)
					}
					ad.setHours(ab, parseInt(ac[3] || 0, 10), parseInt(ac[4]
											|| 0, 10),
							((parseFloat(ac[5] || 0)) || 0) * 1000);
					return ad
				} else {
					return ae
				}
				...
				if (ac) {
					...
					var ab = parseFloat(ac[2]);
					ad.setHours(ab, parseInt(ac[3], 10), parseInt(ac[4], 10),
							parseFloat(ac[5]) * 1000);
					return ad
				} else {
					return ae
				}
				...
				if (ad) {
					...
					var aj = W(
							ad[2],
							aa.regional[aa.regional.getLocale()]["monthNamesShort"]);
					if (aj == -1) {
						aj = W(
								ad[2],
								aa.regional[aa.regional.getLocale()]["monthNames"])
					}
	...
	H.jqplot.sprintf = function() {
		function ah(an, aj, ak, am) {
			var al = (an.length >= aj) ? "" : Array(1 + aj - an.length >>> 0)
					.join(ak);
		...
		function ae(al) {
			...
			for (var aj = 10; aj > 0; aj--) {
				if (ak == (ak = ak.replace(/^(\d+)(\d{3})/, "$1"
								+ H.jqplot.sprintf.thousandsSeparator + "$2"))) {
					break
				}
			}
			return ak
		...
		function ad(ao, an, aq, al, am, ak) {
			...
			if (ap > 0) {
				...
				if (aq || !am) {
					...
					ao = ao.slice(0, an.length) + ah("", ap, "0", true)
							+ ao.slice(an.length)
				}
			}
			return ao
		...
		return ag.replace(H.jqplot.sprintf.regex, function(aG, aq, ar, av, aI,
				aD, ao) {
			if (aG == "%%") {
				return "%"
			}
			...
			switch (ao) {
				...
				case "i" :
					...
					var aB = al < 0 ? "-" : at;
					var aE = an ? ae(String(Math.abs(al))) : String(Math
							.abs(al));
				...
				case "G" :
					...
					var aB = al < 0 ? "-" : at;
					var am = ["toExponential", "toFixed", "toPrecision"]["efg"
							.indexOf(ao.toLowerCase())];
					var aH = ["toString", "toUpperCase"]["eEfFgG".indexOf(ao)
							% 2];
					var aE = Math.abs(al)[am](aD);
					aE = an ? ae(aE) : aE;
					az = aB + aE;
					var aw = ad(az, aB, ax, av, au, ap)[aH]();
					if (H.jqplot.sprintf.decimalMark !== "."
							&& H.jqplot.sprintf.decimalMark !== H.jqplot.sprintf.thousandsSeparator) {
				...
				case "P" :
					...
					var aB = al < 0 ? "-" : at;
					var ay = String(Number(Math.abs(al)).toExponential())
							.split(/e|E/);
					var ak = (ay[0].indexOf(".") != -1)
							? ay[0].length - 1
							: ay[0].length;
...
function _normalizeArguments(b, a, c, d) {
	...
	c = c || a.duration;
	b.duration = $.fx.off ? 0 : typeof c === "number" ? c : c in $.fx.speeds
			? $.fx.speeds[c]
			: $.fx.speeds._default;
...
$.fn.extend({
	jqplotEffect : function(i, j, b, h) {
		var g = _normalizeArguments.apply(this, arguments), d = g.mode, e = g.queue, f = $.jqplot.effects.effect[g.effect], a = !f
				&& backCompat && $.jqplot.effects[g.effect];
		if ($.fx.off || !(f || a)) {
			if (d) {
...
$.jqplot.effects.effect.blind = function(c, h) {
	var d = $(this), k = ["position", "top", "bottom", "left", "right",
			"height", "width"], i = $.jqplot.effects.setMode(d, c.mode
					|| "hide"), m = c.direction || "up", f = rvertical.test(m), e = f
			? "height"
			: "width", j = f ? "top" : "left", p = rpositivemotion.test(m), g = {}, n = i === "show", b, a, l;
