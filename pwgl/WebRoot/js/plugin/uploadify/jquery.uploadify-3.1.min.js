        ...
        init: function(d, e) {
            return this.each(function() {
                ...
                if (h) {
                    ...
                    c("#" + i.movieName).css({
                        position: "absolute",
                        right:"5px",// xyk 与按钮位置发生了偏移,  2014-6-12 新增一个样式
                        "z-index": 1
                    });
                    if (!j.queueID) {
                        var k = c("<div />", {
                ...
                if (e[0]) {
                    if (e[0] == "*") {
                        ...
                        c("#" + j.queueID).find(".uploadify-queue-item").each(function() {
                            ...
                            c(this).find(".data").removeClass("data").html(" - 取消");
                        ...
                        for (var m = 0; m < e.length; m++) {
                            i.cancelUpload(e[m]);
                            c("#" + e[m]).find(".data").removeClass("data").html(" - 取消");
        ...
        onDialogClose: function(d, f, g) {
            ...
            if (c.inArray("onDialogClose", e.overrideEvents) < 0) {
                if (this.queueData.filesErrored > 0) {
                    ctl_showMsg(this.queueData.errorMsg);
                }
            }
            if (e.onDialogClose) {
        ...
        onSelect: function(g) {
            ...
            for (var f in this.queueData.files) {
                ...
                if (e.uploaded != true && e.name == g.name) {
                    var d = confirm('"' + g.name + '" 该文件在列队里已存在,你确定要替换已存在的文件?');
                    if (!d) {
...
//                        this.queueData.errorMsg += "\nThe number of files selected exceeds the remaining upload limit (" + f + ").";
                    	...
                    	//this.queueData.errorMsg += "\nThe number of files selected exceeds the remaining upload limit ("+ f + ").";
                        this.queueData.errorMsg += "一次最多只能选择 "+ e.queueSizeLimit+"个文件上传";
                    }
                    break;
                case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                    //this.queueData.errorMsg += '\nThe file "' + d.name + '" exceeds the size limit (' + e.fileSizeLimit + ").";
                    this.queueData.errorMsg += d.name+",该文件大小已超过"+e.fileSizeLimit+"KB"+"，请重新上传!";
                    break;
