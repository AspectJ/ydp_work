 ...
 * 修改记录：
 * 修改人：任托宇
 * 1、每行的复选框    标记（rentuoyu0001）
 * 2、表头复选框     标记（rentuoyu0002）
 * 3、是否需要增强全选、反选、复选  标记（rentuoyu0003）
 * 4、当该列值为空时，设置为默认值  标记（rentuoyu0004）
 * 5、jqgrid_content_表格id 添加id 
 * 6、(2012-11-28修改记录)处理表格标题可收缩图标显示在左边=========================================
 * 7、2012-12-03修改后，表格滚动加载每次只取一页数据
 * 8、2013.1.29 改动在工具栏上准许加入汇总信息,新增了两个入参html和htmlText，同时将其他工具栏的按钮默认改成false
 * 9、2013-04-02,jqgrid取数据时后台抛出自定义异常时处理
 */
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			loadonce: false,
			multiselect: false,
			needEnhanceCbox:false,//rentuoyu0003  是否需要增强全选、反选、复选
			multikey: false,
			editurl: null,
			search: false,
			caption: "",
			...
			populateVisible: function() {
				...
				if (npage) {
					...
					if (grid.hDiv.loading) {
						...
						grid.populate(1);
						//grid.scrollTop=0;
						//alert((page-1)*grid.prevRowHeight*table[0].rows.length);
						//设置下拉滚动条的位置
						grid.bDiv.scrollTop=grid.prevRowHeight*table[0].rows.length-150;
						//alert(grid.hDiv.loading);
						//---------2012-12-03修改后-------------------------------------------//
					}
				}
			},
		...
		addMulti = function(rowid,pos,irow){//alert("标记（rentuoyu0001） 这里是每行的复选框");
				...
				case "script":
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							...
							//2013-04-02,jqgrid取数据时后台抛出自定义异常时处理------------------
							if(data!=null&&typeof(data)!="undefined"&&data!=""){
								var successFlag=getRequestSuccessFlag(data);
								if(!successFlag){
									data.data=null;
									//$($t).trigger("reloadGrid");
									//表格数据加载失败，请重试或联系系统管理员！
									ctl_showMsg("表格数据加载失败，请重试或联系系统管理员！");
								}
							}
							//---------------------------------------------------------
		...
		//表头复选框
		if(this.p.multiselect) {//alert("标记（rentuoyu0002）");
			var selectHtml="";
			var addWidth=0;
			
			if(!ts.p.needEnhanceCbox){
				//不需要增强checkbox
				selectHtml=selectHtml+"<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/>";
			}else{
				//需要增强checkbox
				selectHtml=selectHtml+"<div class='pro_select'>";
				selectHtml=selectHtml+"<div id='pro_qty_"+this.p.id+"' class='pro_qty'>";
				selectHtml=selectHtml+"<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' style='width:11px;height:11px;' type='checkbox'/>";
				selectHtml=selectHtml+"</div>";
				selectHtml=selectHtml+"<div id='pro_qty_but_"+this.p.id+"' class='pro_qty_but' onclick='show_select(\"pro_qty_but_"+this.p.id+"\",\"qty_items_"+this.p.id+"\",\""+this.p.id+"\")'>";
				selectHtml=selectHtml+"<img src='css/jqgrid/images/cboxSelect.jpg' width='16px' height='16px'/>";
				selectHtml=selectHtml+"</div>";
				selectHtml=selectHtml+"<div class='clear'></div>";
				
				
//				selectHtml=selectHtml+"<div id='qty_items_"+this.p.id+"' class='qty_items' style='display: none;'>";
//				selectHtml=selectHtml+"<div class='qty_items_out'>全选</div>";
//				selectHtml=selectHtml+"<div class='qty_items_out'>反选</div>";
//				selectHtml=selectHtml+"<div class='qty_items_out'>不选</div>";
//				selectHtml=selectHtml+"</div>";
//				selectHtml=selectHtml+"</div>";
				
				addWidth=20;//需要增强checkbox时加宽
			}
			
			this.p.colNames.unshift(selectHtml);
			this.p.colModel.unshift({name:'cb',width:$.jgrid.cellWidth() ? ts.p.multiselectWidth+ts.p.cellLayout+addWidth+'px' :ts.p.multiselectWidth+addWidth+'px' ,sortable:false,resizable:false,hidedlg:true,search:false,align:'center',fixed:true});
			//特别注意：IE、火狐、谷歌  $.jgrid.cellWidth()为false，但是 平板的浏览器为true
		}
		...
		//-----(2012-11-27修改记录)(开始)-----------------------------------------//
		//加入id
		grid.bDiv.id='jqgrid_content_'+$.jgrid.jqID(ts.p.id);
		//-----(2012-11-27修改记录)(结束)-----------------------------------------//
...
//		.append("<span class='ui-icon ui-icon-circle-triangle-n'></span>").css((dir=="rtl"?"right":"left"),"0px") : "";
...
//		.addClass("ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix");
		...
		//					.css("background",'url("css/skin/redmond/images/ui-bg_flat_0_aaaaaa_40x100.png") repeat-x scroll 50% 50% rgb(92, 156, 204)');
		
		//css/skin/redmond/images   ui-bg_flat_0_aaaaaa_40x100.png  ui-bg_flat_55_fbec88_40x100.png
		//url("images/ui-bg_gloss-wave_55_5c9ccc_500x100.png") repeat-x scroll 50% 50% rgb(92, 156, 204);
				...
				if (ln && ln>0){
					while(i<ln){
						if($($t.rows[i]).hasClass('jqgrow')) {
							...
							if(mathopr) {
								v = parseFloat(val);
								if(v.toString()=='NaN'){	//如果为NaN则取0 唐可
									v=0;
								}
								sum += v;
								if(i===0) {
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						...
						.css({'height':'35px', 'border-top': '0px none'})
						.html(titleText);//合并表头的行高由22改成35 pch 2015.12.23
					...
					if (skip === 0) {
						if (o.useColSpanStyle) {
							...
							$th.attr("rowspan", "3");	//多级表头行合并，如果有4级表头此处改为4 唐可
						} else {
...
"use strict";	
	...
	$.fn.fmatter.defaultFormat = function(cellval, opts) {
		...
		//-------------------------------------------------
		//标记（rentuoyu0004）
		//add by rentuoyu  2012年7月13日
		//当该列值为空时，设置为默认值
		//调用方式：formatter:'defaultFormat',empty:'默认值'
		if(opts.colModel !== undefined && !$.fmatter.isUndefined(opts.colModel.empty)){
			defValue=opts.colModel.empty;
		}
		//-------------------------------------------------
		return ($.fmatter.isValue(cellval) && cellval!=="" ) ?  cellval : opts.defaultValue ? opts.defaultValue : defValue;
	};
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				//2013.1.29 改动在工具栏上准许加入汇总信息,新增了两个入参html和htmlText，同时将其他工具栏的按钮默认改成false
				if(o.html){
					tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
					$(tbd).append("<div class='ui-pg-div ui-pg-grid-html'>"+o.htmlText+"</div>");
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
						...
						if(cm[i].editable===true) {
							//update by liying 2014-03-06 
							//修改IE下编辑表格单元格因为获取焦点的原因，导致表格滚动条一直往下滚的问题
