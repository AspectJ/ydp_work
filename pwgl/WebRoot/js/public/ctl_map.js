...
function Map() {
    ...
    // 获取值副本
    this.get2 = function(_key) {
    	 try {
             for (var i = 0; i < this.elements.length; i++) {
                 if (this.elements[i].key == _key) {
                	 var data = null;
                	 var vl = this.elements[i].value;
                	 var type = Object.prototype.toString.call(vl);
    ...
    //xyk  2013-9-23 获取value的属性和属性值 存在的个数  同时满足对象所有属性的对应的值
    // 注意  有一个条件不符合就不行  是并且的概念 
    this.containsValueProperty = function(obj,notContainObj){
    	 var keys = [];
    	 obj = obj || {};
         for (var i = 0; i < this.elements.length; i++) {
        	 var valueObj = this.elements[i].value;
        	 ...
        	 if(notContainObj){
        		 //不包含哪些条件
            	 for(var name2 in notContainObj){
            		 if(!valueObj.hasOwnProperty(name2)){
            			 continue;
            		 }
            		 if(valueObj[name2] == notContainObj[name2]){
            			 countFlag = false;
            		 }
            	 }
        	 }
