...
var pagesize = 10;
var pagingsize = 3;
var theatertype = localStorage.getItem("theatertype"); //3表示即影投  1表示易得票
//消费渠道默认为线下消费
var online = 0; 
//默认时间
	...
	$(".online").on("click", function(){
		...
		if(online == 1) {   		//线上消费
			...
			$(".theater_list_filter li").on("click", function(){
				...
				$(".drp-timeline-presets li:eq(1)").click(); //重置日期
			});
			...
			switch (result) {
				case 1000:
					...
					$(".theater_list_filter li").on("click", function(){
						...
						$(".drp-timeline-presets li:eq(1)").click();
					});
					break;
...
function echoData(data) {
	//初始化数据(清空数据)
	$(".consume_table table:first tr:not(:first)").remove();
	$(".consume_table table:last").hide();
	$(".data-tab-ul").empty();
	//如果有“暂无消费记录”，则清除
	...
	if(resultData != null && resultData.length > 0) {
		for(var i = 0; i < resultData.length; i++) {
			...
			$(".consume_table table:eq(0)").append(html_data);
		...
		$("p[class='statistics']").before(html);
	}
}
...
function fill_sum_data(param, data) {
	...
	paramObj.lastTime = param.lastTime;
	
	var str_obj = JSON.stringify(paramObj); 
	
	
	$(".content-data table:last tr .theaterName").html(param.theaterName);
	$(".content-data table:last tr .beginTime").html(param.beginTime);
	$(".content-data table:last tr .endTime").html(param.endTime);
	$(".content-data table:last tr .SUM").html(data.SUM);
	$(".content-data table:last tr .create_bill").html("<span class='bill_soldout' onclick='create_bill("+ str_obj +")'><b>生成对账单</b></span>");
...
function create_bill(param) {
	...
	if(!isAfterLastBillTime(param.logicBeginTime, param.beginTime)) {
		...
		alert("该影院上一笔账单已生成到" + param.lastTime + " ; 请选择" + param.logicBeginTime + "及其之后的时间");
...
function isBeforeToday(time) {
	...
	var yedayMILL = new Date(y, m-1, d, 23, 59, 59).getTime();
...
function isAfterLastBillTime(LogicBeginTime,actualBeginTime) {
	...
	var logicMILL = new Date(y1, m1-1, d1, 0, 0, 0).getTime(); //逻辑上该笔对账单的理应最先开始时间(上一笔对账单的结束时间+1秒)
	
	
	var y2 = actualBeginTime.substring(0,4); 
	var m2 = actualBeginTime.substring(5,7); 
	var d2 = actualBeginTime.substring(8,10);
	var actualMILL = new Date(y2, m2-1, d2, 0, 0, 0).getTime(); //操作员选择生成对账单的开始时间
