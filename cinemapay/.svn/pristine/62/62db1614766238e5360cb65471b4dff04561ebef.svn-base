...
//确认(显示信息，来自哪个函数【1、用户禁用、启用；2、重置用户密码；3、锁定、激活角色；4、删除角色；5、删除权限；6、删除卡券；7、生成卡号；8、禁用/启用批次；9、禁用卡号；10、启用卡号】，参数1，参数2)
	...
	$("#sure").on("click", function(){
		removeEle( 50, 200, $("#mask"), function(){
			if(type == 1){
				...
				changeConfStatus(param1,param2);
			}else if(type == 9){
				changeCardStatus(param1,param2,2);
			}else if(type == 10){
				changeCardStatus(param1,param2,1);
			}
		});
...
function operateCheckTip(tip, message, attention, sure_fn){
	var msgHtml = '<div class="mask" id="yzm" style="display:block;">';
	...
	// 生成验证码
	var vcode = new vCode($(vCodeImg)[0]);
	...
	$(".deny").on("click", function(){ $("#yzm").remove(); });
	$("#vaCode").focus();
}
