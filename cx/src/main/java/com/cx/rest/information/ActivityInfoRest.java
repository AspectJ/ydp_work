...
public class ActivityInfoRest extends BaseServlet
{
	...
	public String addActivity(@Context HttpServletRequest request, @Context HttpServletResponse response) throws UnsupportedEncodingException{
		...
		String acti_content = URLDecoder.decode(request.getParameter("acti_content"), "UTF-8");
		...
		try{
			...
			actiMap.put("start_time",  new SimpleDateFormat("yyyy-MM-dd").parse(start_time.replaceAll("/", "-")));
			actiMap.put("end_time", new SimpleDateFormat("yyyy-MM-dd").parse(end_time.replaceAll("/", "-")));
			actiMap.put("author", author);
			...
			data.put("start_time", start_time.replaceAll("/", "-"));
			data.put("end_time", end_time.replaceAll("/", "-"));
			data.put("cinemaid", resultMap.get("cinemaid"));
			data.put("audit_flag", "0");
			data.put("author", author);
			data.put("acti_img", Integer.parseInt(acti_img_id));
			data.put("org_path", acti_img);
	...
	public String updateActivity(@Context HttpServletRequest request, @Context HttpServletResponse response) throws UnsupportedEncodingException{
		...
		try{
			...
			MyMongo.mErrorLog("修改最新活动信息失败", request, e);
			return this.returnError(resultJson, ResMessage.Server_Abnormal.code, request);
		}
		
		// -------------------------------------------------------------------------------
		long etime = System.currentTimeMillis();
		MyMongo.mRequestLog("修改最新活动信息成功",  etime - stime, request, resultJson);
		return this.response(resultJson, request);
	}
...
//				String acti_status = CodeUtil.convertNewsStatus(resultMap.get("acti_status").toString());
				data.put("acti_status_name", resultMap.get("acti_status"));
				data.put("cinemaid", resultMap.get("cinemaid"));
	...
	public String getActivityList(@Context HttpServletRequest request, @Context HttpServletResponse response){
		...
		String audit_flag = request.getParameter("audit_flag");//查询审核通过的标志
		try{
			Map<String, Object> actiInfo = new HashMap<String, Object>();
			...
			int count = activityDao.getActivityListCount(actiInfo);
			if(count > 0){
				...
				for(int i=0; i<resultList.size(); i++){
					...
					jsonObject.put("acti_status", resultMap.get("acti_status"));
