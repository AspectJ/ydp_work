...
public class AdminInfoRest extends BaseServlet{
		...
		try {
			...
			if(count > 0){
				...
				for(int i = 0; i<adminList.size(); i++) {
					...
					if(resultMap.containsKey("last_login_time")) {
						jsonObject.put("last_login_time", new SimpleDateFormat("yyyy-MM-dd HH:mm").format(resultMap.get("last_login_time")));
					}
	...
	public String addAdminInfo(@Context HttpServletRequest request, @Context HttpServletResponse response) throws UnsupportedEncodingException {
		...
		try {
			if (CodeUtil.checkParam(adminname, nickname, adminpwd, cinemaid,state)){
				MyMongo.mRequestFail(request, ResMessage.Lack_Param.code);
				return this.returnError(resultJson, ResMessage.Lack_Param.code, request);
			}else {
			        ...
			        adminInfoMap.put("state", "true".equals(state) ? 1 : 0);
					//查询管理员账号是否已存在
					...
					if(adminMap != null) {
						...
						//------------------------------------------------------------------------
						Map<String, Object> receiveMap = adminInfoDao.selSingleAdmin(resultMap);
						//获取管理员被创建时间（必须是字符串类型的）
						String create_time = new SimpleDateFormat("yyyy-MM-dd HH:mm").format(receiveMap.get("create_time"));
						receiveMap.put("create_time", create_time);
						//获取管理员最后一次登录时间（必须是字符串类型的）
						if(receiveMap.containsKey("last_login_time")) {
							String last_login_time = new SimpleDateFormat("yyyy-MM-dd HH:mm").format(receiveMap.get("last_login_time"));
							receiveMap.put("last_login_time", last_login_time);
						}
					     	receiveMap.put("cinemaname", receiveMap.get("name"));
							receiveMap.put("audit", receiveMap.get("audit_flag"));
							
						//------------------------------------------------------------------------
						resultJson.put("data", receiveMap);
					}
			}
		}catch(Exception e) {
...
//		String cinema_id = request.getParameter("cinema_id");
		...
		try {
			if (CodeUtil.checkParam(admin_name, admin_nickname, admin_state)){
				MyMongo.mRequestFail(request, ResMessage.Lack_Param.code);
				return this.returnError(resultJson, ResMessage.Lack_Param.code, request);
			}
...
//			adminInfoMap.put("cinemaid", Integer.parseInt(cinema_id));
			...
			adminInfoMap.put("state", "true".equals(admin_state) ? 1 : 0);
	        adminInfoMap.put("audit", "true".equals(audit) ? 1 : 0);
